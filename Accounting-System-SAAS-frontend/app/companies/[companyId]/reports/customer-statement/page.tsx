...
export default function CustomerStatementReportPage() {
  ...
  const { report, isLoading, error } = useCustomerStatement(
    companyId,
    submittedCustomerId,
    submittedFromDate,
    submittedToDate,
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between px-4 py-2">
          <div className="flex items-center gap-3">
            ...
            >
              <svg className="w-4 h-4 text-slate-500 group-hover:text-indigo-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>
            </button>
            <div>
              <h1 className="text-sm font-bold text-slate-800 dark:text-slate-100 tracking-tight">Customer Statement</h1>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-tight">Customer account outstanding & payment history</p>
            </div>
          </div>
      ...
      <form onSubmit={handleApply} className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-3 text-xs shadow-sm flex flex-wrap items-end gap-4 mt-1">
        <div className="flex flex-col gap-1">
          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wide">Customer ID</label>
          <input
            type="text"
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            ...
            onChange={(e) => {
              const next = (e.target.value as string).split(' ')[0] as any;
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="h-9 px-6 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs shadow-md transition-all active:scale-95 disabled:opacity-50"
        >
      ...
      <div ref={printRef} className="rounded-xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900 shadow-sm p-4 text-sm mt-1 min-h-[400px]">
        {error && (
          <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-xs text-red-700 flex items-center gap-2">
            <span>⚠️</span>
            <span>{error?.response?.data?.detail || "Failed to load statement. Please verify the Customer ID."}</span>
          </div>
        )}
        ...
        {!isLoading && !error && !report && (
          <div className="flex flex-col items-center justify-center py-20 text-slate-400 gap-4 opacity-70">
            ...
            <div className="text-center">
              <p className="font-bold text-slate-600 dark:text-slate-300">Customer Statement Viewer</p>
              <p className="text-xs mt-1">Enter a valid Customer ID and select your date range to begin.</p>
            </div>
          </div>
        )}
        
        {isLoading && (
          <div className="flex flex-col items-center justify-center py-20 text-indigo-500 animate-pulse gap-4">
            <div className="w-10 h-10 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin" />
            <p className="text-[10px] font-black uppercase tracking-[0.2em]">Synchronizing Financial Data...</p>
          </div>
        )}
      </div>
    </div>
